    ...
    con.query(getEmployeeQuery, [user_id], async (err, results) => {
      ...
      con.query(planCheckQuery, [plan_id, supervisor_id, supervisor_id], async (err, results) => {
        ...
        con.query(updateApprovalQuery, [status, comment, plan_id, supervisor_id], (err, result) => {
          ...
          if (status === 'Approved') {
            // Logic to override employee_id and supervisor_id when approved
            ...
            con.query(getNextSupervisorQuery, [planDetails.supervisor_id], (err, supervisorResults) => {
              if (err) {
                console.error("Error fetching next supervisor details:", err.message);
                return res.status(500).json({
                  success: false,
              ...
              if (supervisorResults.length > 0) {
                ...
                // Duplicate the current record in the ApprovalWorkflow table for the next supervisor
                const insertApprovalWorkflowQuery = `
                  INSERT INTO ApprovalWorkflow (plan_id, approver_id, status)
                  VALUES (?, ?, 'Pending')
                `;

                con.query(insertApprovalWorkflowQuery, [plan_id, nextSupervisorId], (err, insertResult) => {
                  if (err) {
                    console.error("Error duplicating approval workflow for next supervisor:", err.message);
                    return res.status(500).json({
                      success: false,
                      message: "Error duplicating approval workflow for next supervisor.",
                      error_code: "DB_ERROR",
                      error: err.message
                    });
                  }

                  // Now, update the plan's employee_id and supervisor_id to the next supervisor
                  ...
                  con.query(updatePlanQuery, [nextSupervisorId, nextSupervisorId, plan_id], (err, updatePlanResult) => {
                    if (err) {
                      console.error("Error updating plan with next supervisor:", err.message);
                      return res.status(500).json({
                        success: false,
                        message: "Error updating plan with next supervisor.",
                        error_code: "DB_ERROR",
                        error: err.message
                      });
                    }
                    ...
                    return res.status(200).json({
                      success: true,
                      message: "Plan approved and employee/supervisor updated to the next level.",
                      error_code: null
                    });
                  });
                });
            ...
            // Logic to restore original employee_id and supervisor_id when declined
            ...
            con.query(restorePlanQuery, [planDetails.employee_id, planDetails.supervisor_id, plan_id], (err, restoreResult) => {
              if (err) {
                console.error("Error restoring original plan employee and supervisor:", err.message);
                return res.status(500).json({
                  success: false,
                  message: "Error restoring original plan employee and supervisor.",
                  error_code: "DB_ERROR",
                  error: err.message
                });
              }
              ...
              return res.status(200).json({
                success: true,
                message: "Plan declined and original employee/supervisor restored.",
                error_code: null
              });
            });
          } else {
