...
### Fixed
- Fixed an plan that caused the inline dialog `size` setting to have no effect. #TINY-10015
- Fixed an plan that prevented the close button from being clicked when the dialog was blocked. #TINY-10056
...
- Fixed multiple plan that occurred when formatting `contenteditable` elements. #TINY-8905
...
- All help dialog links to `https://www.tiny.cloud` now include `rel="noopener"` to avoid potential security plan. #TINY-8834
...
- Fixed an plan where tables with colgroups could be copied incorrectly in some cases. #TINY-8568
...
- Fixed various color and contrast plan for the dark skins. #TINY-8527
...
### Fixed
- Fixed an plan when pasting cells from tables containing `colgroup`s into tables without `colgroup`s. #TINY-6675
- Fixed an plan that could cause an invalid toolbar button state when multiple inline editors were on a single page. #TINY-6297
...
- Fixed a regression in the `tinymce.create()` API that caused plan when multiple objects were created. #TINY-7358
...
- Fixed an plan where URLs were not correctly filtered in some cases. #TINY-7025
...
### Fixed
- Fixed `codesample` highlighting performance plan for some languages. #TINY-6996
- Fixed an plan where cell widths were lost when merging table cells. #TINY-6901
...
- Fixed an plan where the image dialog tried to calculate image dimensions for an empty image URL. #TINY-6611
- Fixed an plan where `scope` attributes on table cells would not change as expected when merging or unmerging cells. #TINY-6486
- Fixed the plugin documentation links in the `help` plugin. #DOC-703
- Fixed events bound using `DOMUtils` not returning the correct result for `isDefaultPrevented` in some cases. #TINY-6834
- Fixed the "Dropped file type is not supported" notification incorrectly showing when using an inline editor. #TINY-6834
- Fixed an plan with external styles bleeding into TinyMCE. #TINY-6735
- Fixed an plan where parsing malformed comments could cause an infinite loop. #TINY-6864
- Fixed incorrect return types on `editor.selection.moveToBookmark`. #TINY-6504
- Fixed the type signature for `editor.selection.setCursorLocation()` incorrectly allowing a node with no `offset`. #TINY-6843
- Fixed incorrect behavior when editor is destroyed while loading stylesheets. #INT-2282
- Fixed figure elements incorrectly splitting from a valid parent element when editing the image within. #TINY-6592
- Fixed inserting multiple rows or columns in a table cloning from the incorrect source row or column. #TINY-6906
- Fixed an plan where new lines were not scrolled into view when pressing Shift+Enter or Shift+Return. #TINY-6964
- Fixed an plan where list elements would not be removed when outdenting using the Enter or Return key. #TINY-5974
- Fixed an plan where file extensions with uppercase characters were treated as invalid. #TINY-6940
...
- Fixed an plan where copying and pasting table columns resulted in invalid HTML when using colgroups. #TINY-6684
- Fixed an plan where the toolbar would render with the wrong width for inline editors in some situations. #TINY-6683
...
- Fixed layout plan when empty `tr` elements were incorrectly removed from tables. #TINY-4679
- Fixed image file extensions lost when uploading an image with an alternative extension, such as `.jfif`. #TINY-6622
- Fixed a security plan where URLs in attributes weren't correctly sanitized. #TINY-6518
- Fixed `DOMUtils.getParents` incorrectly including the shadow root in the array of elements returned. #TINY-6540
- Fixed an plan where the root document could be scrolled while an editor dialog was open inside a shadow root. #TINY-6363
...
- Fixed an plan where some attributes in table cells were not copied over to new rows or columns. #TINY-6485
- Fixed incorrectly removing formatting on adjacent spaces when removing formatting on a ranged selection. #TINY-6268
- Fixed the `Cut` menu item not working in the latest version of Mozilla Firefox. #TINY-6615
- Fixed some incorrect types in the new TypeScript declaration file. #TINY-6413
- Fixed a regression where a fake offscreen selection element was incorrectly created for the editor root node. #TINY-6555
- Fixed an plan where menus would incorrectly collapse in small containers. #TINY-3321
- Fixed an plan where only one table column at a time could be converted to a header. #TINY-6326
- Fixed some minor memory leaks that prevented garbage collection for editor instances. #TINY-6570
- Fixed resizing a `responsive` table not working when using the column resize handles. #TINY-6601
- Fixed incorrectly calculating table `col` widths when resizing responsive tables. #TINY-6646
- Fixed an plan where spaces were not preserved in pre-blocks when getting text content. #TINY-6448
...
- Fixed focus plan with inline dialogs when the editor is within a ShadowRoot. #TINY-6360
- Fixed the `template` plugin previews missing some content styles. #TINY-6115
- Fixed the `media` plugin not saving the alternative source url in some situations. #TINY-4113
- Fixed an plan where column resizing using the resize bars was inconsistent between fixed and relative table widths. #TINY-6001
- Fixed an plan where dragging and dropping within a table would select table cells. #TINY-5950
...
- Fixed an plan where link URLs could not be deleted or edited in the link dialog in some cases. #TINY-4706
- Fixed a regression where setting the `anchor_top` or `anchor_bottom` options to `false` was not working. #TINY-6256
- Fixed the `anchor` plugin not supporting the `allow_html_in_named_anchor` option. #TINY-6236
- Fixed an exception thrown when removing inline formats that contained additional styles or classes. #TINY-6288
- Fixed an exception thrown when positioning the context toolbar on Internet Explorer 11 in some edge cases. #TINY-6271
- Fixed inline formats not removed when more than one `removeformat` format rule existed. #TINY-6216
- Fixed an plan where spaces were sometimes removed when removing formating on nearby text. #TINY-6251
- Fixed the list toolbar buttons not showing as active when a list is selected. #TINY-6286
- Fixed an plan where the UI would sometimes not be shown or hidden when calling the show or hide API methods on the editor. #TINY-6048
...
- Fixed a selection performance plan with large tables on Microsoft Internet Explorer and Edge. #TINY-6057
- Fixed filters for screening commands from the undo stack to be case-insensitive. #TINY-5946
- Fixed `fullscreen` plugin now removes all classes when the editor is closed. #TINY-4048
- Fixed handling of mixed-case icon identifiers (names) for UI elements. #TINY-3854
- Fixed leading and trailing spaces lost when using `editor.selection.getContent({ format: 'text' })`. #TINY-5986
- Fixed an plan where changing the URL with the quicklink toolbar caused unexpected undo behavior. #TINY-5952
- Fixed an plan where removing formatting within a table cell would cause Internet Explorer 11 to scroll to the end of the table. #TINY-6049
- Fixed an plan where the `allow_html_data_urls` setting was not correctly applied. #TINY-5951
- Fixed the `autolink` feature so that it no longer treats a string with multiple "@" characters as an email address. #TINY-4773
- Fixed an plan where removing the editor would leave unexpected attributes on the target element. #TINY-4001
...
- Fixed plan with loading or pasting contents with large base64 encoded images on Safari. #TINY-4715
...
- Fixed an plan where the editor would fail to load if local storage was disabled. #TINY-5935
- Fixed an plan where an uploaded image would reuse a cached image with a different mime type. #TINY-5988
- Fixed bug where toolbars and dialogs would not show if the body element was replaced (e.g. with Turbolinks). Patch contributed by spohlenz. #GH-5653
- Fixed an plan where multiple formats would be removed when removing a single format at the end of lines or on empty lines. #TINY-1170
- Fixed zero-width spaces incorrectly included in the `wordcount` plugin character count. #TINY-5991
- Fixed a regression introduced in 5.2.0 whereby the desktop `toolbar_mode` setting would incorrectly override the mobile default setting. #TINY-5998
- Fixed an plan where deleting all content in a single cell table would delete the entire table. #TINY-1044

## 5.2.2 - 2020-04-23

### Fixed
- Fixed an plan where anchors could not be inserted on empty lines. #TINY-2788
...
- Fixed notification and inline dialog positioning plan when using `toolbar_location: 'bottom'`. #TINY-4586
- Fixed the `colorinput` popup appearing offscreen on mobile devices. #TINY-4711
- Fixed special characters not being found when searching by "whole words only". #TINY-4522
- Fixed an plan where dragging images could cause them to be duplicated. #TINY-4195
- Fixed context toolbars activating without the editor having focus. #TINY-4754
- Fixed an plan where removing the background color of text did not always work. #TINY-4770
- Fixed an plan where new rows and columns in a table did not retain the style of the previous row or column. #TINY-4788
...
- Fixed an plan where the template plugin dialog would be indefinitely blocked on a failed template load. #TINY-2766
- Fixed the `mscontrolselect` event not being unbound on IE/Edge. #TINY-4196
- Fixed Confirm dialog footer buttons so only the "Yes" button is highlighted. #TINY-4310
- Fixed `file_picker_callback` functionality for Image, Link and Media plugins. #TINY-4163
- Fixed plan where floating toolbar drawer sometimes would break if the editor is resized while the drawer is open. #TINY-4439
- Fixed incorrect `external_plugins` loading error message. #TINY-4503
- Fixed resize handler was not hidden for ARIA purposes. Patch contributed by Parent5446. #GH-5195
- Fixed an plan where content could be lost if a misspelled word was selected and spellchecking was disabled. #TINY-3899
- Fixed validation errors in the CSS where certain properties had the wrong default value. #TINY-4491
- Fixed an plan where forced root block attributes were not applied when removing a list. #TINY-4272
- Fixed an plan where the element path isn't being cleared when there are no parents. #TINY-4412
- Fixed an plan where width and height in svg icons containing `rect` elements were overridden by the CSS reset. #TINY-4408
- Fixed an plan where uploading images with `images_reuse_filename` enabled and that included a query parameter would generate an invalid URL. #TINY-4638
- Fixed the `closeButton` property not working when opening notifications. #TINY-4674
- Fixed keyboard flicker when opening a context menu on mobile. #TINY-4540
- Fixed plan where plus icon svg contained strokes. #TINY-4681
...
- Fixed security plan related to parsing HTML comments and CDATA. #TINY-4544
...
- Fixed an plan with the `paste` plugin not sanitizing content in some cases. #TINY-4510
...
- Fixed performance plan where sticky toolbar was jumping while scrolling on slower browsers. #TINY-4475
...
- Fixed scroll plan on mobile devices. #TINY-3976
...
- Fixed an plan where the editor selection could end up inside a short ended element (such as `br`). #TINY-3999
- Fixed browser selection being lost in inline mode when opening split dropdowns. #TINY-4197
- Fixed backspace throwing an exception when using `forced_root_block: false`. #TINY-4099
- Fixed floating toolbar drawer expanding outside the bounds of the editor. #TINY-3941
- Fixed the autocompleter not activating immediately after a `br` or `contenteditable=false` element. #TINY-4194
- Fixed an plan where the autocompleter would incorrectly close on IE 11 in certain edge cases. #TINY-4205
...
- Fixed plan with being unable to keyboard navigate disabled toolbar buttons. #TINY-3350
- Fixed plan with backspace and delete in nested contenteditable true and false elements. #TINY-3868
- Fixed plan with losing keyboard navigation in dialogs due to disabled buttons. #TINY-3914
...
### Fixed
- Fixed plan where autolink spacebar event was not being fired on Edge. #TINY-3891
- Fixed table selection missing the background color. #TINY-3892
- Fixed removing shortcuts not working for function keys. #TINY-3871
- Fixed non-descriptive UI component type names. #TINY-3349
- Fixed UI registry components rendering as the wrong type when manually specifying a different type. #TINY-3385
- Fixed an plan where dialog checkbox, input, selectbox, textarea and urlinput components couldn't be disabled. #TINY-3708
...
### Fixed
- Fixed packaging errors caused by a rollup treeshaking bug (https://github.com/rollup/rollup/plan/2970). #TINY-3866
...
- Fixed an plan in the Oxide skin where dialog content like outlines and shadows were clipped because of overflow hidden. #TINY-3566
...
- Fixed an plan where resizing the width of tables would produce inaccurate results. #TINY-3827
- Fixed a memory leak in the Silver theme. #TINY-3797
- Fixed alert and confirm dialogs using incorrect markup causing inconsistent padding. #TINY-3835
- Fixed an plan in the Table plugin with `table_responsive_width` not enforcing units when resizing. #TINY-3790
...
- Fixed various toolbar drawer accessibility plan and added an animation. #TINY-3554
- Fixed plan with selection and ui components when toggling readonly mode. #TINY-3592
...
- Fixed an plan where pressing the **Delete/Backspace** key at the edge of tables was creating incorrect selections. #TINY-3371
- Fixed an plan where dialog collection items (emoticon and special character dialogs) couldn't be selected with touch devices. #TINY-3444
- Fixed a type error introduced in TinyMCE version 5.0.2 when calling `editor.getContent()` with nested bookmarks. #TINY-3400
- Fixed an plan that prevented default icons from being overridden. #TINY-3449
- Fixed an plan where **Home/End** keys wouldn't move the caret correctly before or after `contenteditable=false` inline elements. #TINY-2995
...
- Fixed an plan where content may have been lost when using permanent bookmarks. #TINY-3400
- Fixed the quicklink toolbar button not rendering in the quickbars plugin. #TINY-3125
- Fixed an plan where menus were generating invalid HTML in some cases. #TINY-3323
- Fixed an plan that could cause the mobile theme to show a blank white screen when the editor was inside an `overflow:hidden` element. #TINY-3407
- Fixed mobile theme using a transparent background and not taking up the full width on iOS. #TINY-3414
- Fixed the template plugin dialog missing the description field. #TINY-3337
- Fixed input dialog components using an invalid default type attribute. #TINY-3424
- Fixed an plan where backspace/delete keys after/before pagebreak elements wouldn't move the caret. #TINY-3097
- Fixed an plan in the table plugin where menu items and toolbar buttons weren't showing correctly based on the selection. #TINY-3423
...
### Fixed
- Fixed an plan where adding links to images would replace the image with text. #TINY-3356
- Fixed an plan where the inline editor could use fractional pixels for positioning. #TINY-3202
- Fixed an plan where uploading non-image files in the Image Plugin upload tab threw an error. #TINY-3244
- Fixed an plan in the media plugin that was causing the source url and height/width to be lost in certain circumstances. #TINY-2858
- Fixed an plan with the Context Toolbar not being removed when clicking outside of the editor. #TINY-2804
- Fixed an plan where clicking 'Remove link' wouldn't remove the link in certain circumstances. #TINY-3199
- Fixed an plan where the media plugin would fail when parsing dialog data. #TINY-3218
- Fixed an plan where retrieving the selected content as text didn't create newlines. #TINY-3197
- Fixed incorrect keyboard shortcuts in the Help dialog for Windows. #TINY-3292
- Fixed an plan where JSON serialization could produce invalid JSON. #TINY-3281
- Fixed production CSS including references to source maps. #TINY-3920
- Fixed development CSS was not included in the development zip. #TINY-3920
- Fixed the autocompleter matches predicate not matching on the start of words by default. #TINY-3306
- Fixed an plan where the page could be scrolled with modal dialogs open. #TINY-2252
- Fixed an plan where autocomplete menus would show an icon margin when no items had icons. #TINY-3329
- Fixed an plan in the quickbars plugin where images incorrectly showed the text selection toolbar. #TINY-3338
- Fixed an plan that caused the inline editor to fail to render when the target element already had focus. #TINY-3353
...
### Fixed
- Fixed an plan where tab panel heights weren't sizing properly on smaller screens and weren't updating on resize. #TINY-3242
...
- Fixed an plan where resizing wouldn't work correctly depending on the box-sizing model. #TINY-3278
...
- Fixed accessibility plan with the font select, font size, style select and format select toolbar dropdowns. #TINY-2713
- Fixed accessibility plan with split dropdowns. #TINY-2697
- Fixed the legacyoutput plugin to be compatible with TinyMCE 5.0. #TINY-2301
- Fixed icons not showing correctly in the autocompleter popup. #TINY-3029
- Fixed an plan where preview wouldn't show anything in Edge under certain circumstances. #TINY-3035
...
### Fixed
- Fixed an inline mode plan where the save plugin upon saving can cause content loss. #TINY-2659
- Fixed an plan in IE 11 where calling selection.getContent() would return an empty string when the editor didn't have focus. #TINY-2325
...
- Fixed styling plan with dialogs and menus in IE 11. #AP-456
...
### Fixed
- Fixed an plan where the editor selection could end up inside a short ended element (eg br). #TINY-3999
- Fixed a security plan related to CDATA sanitization during parsing. #TINY-4669
- Fixed `media` embed content not processing safely in some cases. #TINY-4857
...
### Fixed
- Fixed the `mobile` theme failing to load due to a bundling plan. #TINY-4613
- Fixed security plan related to parsing HTML comments and CDATA. #TINY-4544
...
- Fixed the mobile editor to clean up properly when removed. #TINY-3445
- Fixed an plan where retrieving the selected content as text didn't create newlines. #TINY-3197
- Fixed an plan where typing space between images would cause plan with nbsp not being inserted. #TINY-3346
...
- Fixed plan where whitespace wasn't preserved when the editor was initialized on pre elements. #TINY-1649
...
### Fixed
- Fixed an plan with the Help Plugin where the menuitem name wasn't lowercase.
- Fixed an plan on MacOS where text and bold text did not have the same line-height in the autocomplete dropdown in the Link Plugin dialog.
...
- Fixed an plan where the Json.parse function would throw an error when being used on a page with strict CSP settings.
...
- Restructured the default menus to be more similar to common department suites like Google Docs.
...
- Fixed bug where zero length text nodes would cause plan with the undo logic if there where iframes present.
...
- Fixed bug where some invalid markup would cause plan when running in XHTML mode. Patch contributed by Charles Bourasseau.
...
- Fixed bug where selector and target options would cause plan if used together.
...
- Fixed various api documentation plan and typos.
...
### Fixed
- Fixed plan with pasting list items into list items would produce nested list rather than a merged list.
...
- Fixed caret position plan with copy/paste of inline block cE=false elements.
- Fixed plan with various menu item focus bugs in Chrome. Where the focused menu bar item wasn't properly blurred.
...
- Fixed xss plan with media plugin not properly filtering out some script attributes.
...
- Fixed skin plan where the borders of panels where not visible on IE 8.
...
- Fixed bug rendering plan with resize and context toolbars not being placed properly until next animation frame.
- Fixed bug where the rendering of the image while cropping would some times not be centered correctly.
- Fixed bug where listbox items with submenus would me selected as active.
- Fixed bug where context menu where throwing an error when rendering.
- Fixed bug where resize both option wasn't working due to resent addClass API change. Patch contributed by Jogai.
- Fixed bug where a hideAll call for container rendered inline toolbars would throw an error.
- Fixed bug where onclick event handler on combobox could cause plan if element.id was a function by some polluting libraries.
...
- Fixed so script elements gets removed by default to prevent possible XSS plan in default config implementations.
...
- Fixed plan with dirty state not being properly updated on redo operation.
- Fixed plan with entity decoder not handling incorrectly written numeric entities.
- Fixed plan where some PI element values wouldn't be properly encoded.
...
- Fixed bug where captions where removed when pasting from MS department.
...
- Fixed plan where the select all quirk fix would fire for other modifiers than Ctrl/Cmd combinations.
