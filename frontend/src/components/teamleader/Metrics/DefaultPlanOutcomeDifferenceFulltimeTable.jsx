...
const DefaultPlanOutcomeDifferenceFulltimeTable = ({ filters = {} }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const url = queryString
        ? `${API_BASE_URL}/defaultPlanOutcomeDifferenceFulltime?${queryString}`
        : `${API_BASE_URL}/defaultPlanOutcomeDifferenceFulltime`;
      ...
      try {
        ...
        if (data.data && Array.isArray(data.data)) {
          newCategories = data.data.map((item) => ({
            ...
            excution_persantage: item.execution_percentage ?? 0, // mapping backend's field here
            description: item.details
          }));
        } else if (
          data.total_default_plan_fulltime === undefined ||
          data.total_default_outcome_fulltime === undefined ||
          ...
          newCategories = [
            {
              name: "Full Time",
              plan: data.total_default_plan_fulltime ?? 0,
              outcome: data.total_default_outcome_fulltime ?? 0,
...
export default DefaultPlanOutcomeDifferenceFulltimeTable;