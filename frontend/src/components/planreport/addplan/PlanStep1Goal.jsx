...
const PlanStep1Goal = ({ onNext }) => {
  ...
  useEffect(() => {
    const fetchgoals = async () => {
      try {
        const response = await axios.get("http://192.168.100.134:5000/api/goalsg", {
          headers: { 
  ...
  const handleCreategoal = async () => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.100.134:5000/api/addgoal",
        {
          name: newgoal.name,
