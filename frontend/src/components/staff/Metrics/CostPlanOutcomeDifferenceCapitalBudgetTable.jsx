...
/**
 * CostPlanOutcomeDifferenceCapitalBudgetTable component fetches cost plan and outcome data
 * for the capital budget, processes the API response, and renders a data table.
 *
 * The component supports both legacy and new backend structures.
 *
 * Legacy Response:
 *   - total_cost_plan_capital
 *   - total_cost_outcome_capital
 *   - difference
 *
 * New Response:
 ...
 */
const CostPlanOutcomeDifferenceCapitalBudgetTable = ({ filters = {} }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const url = queryString
        ? `${API_BASE_URL}/costPlanOutcomeDifferenceCapitalBudget?${queryString}`
        : `${API_BASE_URL}/costPlanOutcomeDifferenceCapitalBudget`;
      ...
      try {
        ...
        if (data.data && Array.isArray(data.data)) {
          ...
          data.total_cost_plan_capital === undefined ||
          data.total_cost_outcome_capital === undefined ||
          ...
          newCategories = [
            {
              name: "Capital Budget",
              plan: data.total_cost_plan_capital ?? 0,
              outcome: data.total_cost_outcome_capital ?? 0,
  ...
  return (
    <div>
      <h2>የ Capital በጀት እቅድ እና ውጤት</h2>
...
export default CostPlanOutcomeDifferenceCapitalBudgetTable;