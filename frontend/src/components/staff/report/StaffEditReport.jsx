...
const StaffEditReport = ({ reportId }) => {
  ...
  const fetchReport = async () => {
    ...
    try {
      const response = await Axios.get(`http://192.168.100.134:5000/api/plans/${reportId}`, {
        headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await Axios.put(
        `http://192.168.100.134:5000/api/plans/${reportId}`,
        formData,
        {
          headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      await Axios.delete(`http://192.168.100.134:5000/api/plans/${reportId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
